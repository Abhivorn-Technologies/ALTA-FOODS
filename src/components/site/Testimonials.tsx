...
const items = [
  {
    n: "Ramesh Patel",
    r: "Mango Orchardist, Gujarat",
    q: "After switching to ALTA FOODS bags, my Kesar mangoes fetch a 30% premium. Cleaner skin, no fly damage.",
    s: 5,
  },
  {
    n: "Anita Sharma",
    r: "Organic Farm, Maharashtra",
    q: "Truly biodegradable, easy to apply, and my pesticide bills dropped by half.",
    s: 5,
  },
  {
    n: "Joseph Mathew",
    r: "Exporter, Kerala",
    q: "Export-grade finish on every shipment. The team customizes sizes for our specific buyers.",
    s: 5,
  },
];
  ...
  return (
    <section className="py-24 bg-paper/40">
      <div className="container-px max-w-4xl mx-auto text-center">
        ...
        <Reveal delay={0.1}>
          <h2 className="mt-4 text-3xl md:text-5xl font-bold">Trusted by growers worldwide.</h2>
        </Reveal>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="mt-5 flex justify-center gap-1">
                {Array.from({ length: items[i].s }).map((_, k) => (
        ...
        <div className="mt-6 flex justify-center gap-2">
          {items.map((_, k) => (
            <button
              ...
              className={`h-2 rounded-full transition-all ${k === i ? "w-8 bg-leaf" : "w-2 bg-muted-foreground/40"}`}
            />
          ))}
        </div>
      </div>
